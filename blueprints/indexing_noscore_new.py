...
shot_schema = Schema(shot_id=ID(unique=True,stored=True),
#first level
color1=TEXT(),color2=TEXT(),color3=TEXT(),color4=TEXT(),color5=TEXT(),color6=TEXT(),
color7=TEXT(),color8=TEXT(),color9=TEXT(),brightness=NUMERIC(float),contrast=NUMERIC(float),saturation=NUMERIC(float),
#second level
...
loc_year=NUMERIC(int,stored=True), loc_month=NUMERIC(int,stored=True), loc_day=NUMERIC(int,stored=True), loc_weekday=KEYWORD(commas=True,scorable=True), loc_hour_start=NUMERIC(int,stored=True), loc_hour_end=NUMERIC(int,stored=True),loc_date=DATETIME(stored=True), 
tags=TEXT(stored=True),past_tags=TEXT(stored=True),future_tags=TEXT(stored=True),
...
#third level
location=TEXT(stored=True),activity=TEXT(stored=True),companion=KEYWORD(commas=True),
is_moving=BOOLEAN(stored=True),small_location=KEYWORD(commas=True,scorable=True,stored=True),
past_location=TEXT(stored=True),past_activity=TEXT(stored=True),past_companion=KEYWORD(commas=True),
past_is_moving=BOOLEAN(stored=True),
future_location=TEXT(stored=True),future_activity=TEXT(stored=True),future_companion=KEYWORD(commas=True),
future_is_moving=BOOLEAN(stored=True)
)

# create index
if not os.path.exists("index_updatemar9_dir"):
    os.mkdir("index_updatemar9_dir")

shot_ix = index.create_in("index_updatemar9_dir", shot_schema)
...
tags_file=open('../../LSC2020_dataset/process_exblur/all_tags_update_mar9.txt','r')
tags_dict = {}
img_utc_time = {}
for line in tags_file:
    ...
    for key in line[2:]:
        ...
        tags_dict[img_name][key[0]].append(('NULL',key[3],key[1]))
...
shot_file.close()
shot_file = open('../../LSC2020_dataset/process_exblur/Shotmrg_newkey.txt','r')
Outfile = open('../../LSC2020_dataset/process_exblur/Shotinfo_search_mar9.txt','w')
cnt = 0
for line in shot_file:
    cnt += 1
    if not line:
    ...
    this_tags = [x[0] for x in sorted(shot_tags[shot_id].items(), key=lambda y:y[1],reverse=True)]
    ...
    # this_past_t = []
    this_past_t = [x[0] for x in sorted(past_shot_tags[shot_id].items(), key=lambda y:y[1],reverse=True)]
    # for t,s in past_shot_tags[shot_id].items():
    #     if s > 0.9:
    #         this_past_t += [t]
    #     this_past_t += [t]#* int(s*5+1)
    this_past_t = ','.join(this_past_t)
    # this_future_t = []
    this_future_t = [x[0] for x in sorted(future_shot_tags[shot_id].items(), key=lambda y:y[1],reverse=True)]
    # for t,s in future_shot_tags[shot_id].items():
    #     if s > 0.9:
    #         this_future_t += [t]
    #     this_future_t += [t]#* int(s*5+1)
    ...
    for k in ['elevation','speed','calories','heart','steps','distance']:
        this_lifelog[k]=[]
    this_lifelog['small_location']=set()
    m = 'is_moving'
    ...
    while m_time <= end_time:
        ...
        this_lifelog['small_location'].add(m_info['small_location'])
    ...
    loc_set = set()
    for loc in this_lifelog['small_location']:
    ...
    this_lifelog['small_location'] = ",".join(list(this_lifelog['small_location'])).lower()
    ...
    Output_line = [str(shot_id),
                   ...
                   this_tags, this_lifelog['location'],this_lifelog['activity'], this_lifelog['small_location'],
    ...
    loc_hour_start =int(loc_stime.hour), loc_hour_end = int(loc_etime.hour), loc_date = loc_stime,
    ...
    #third level
    location = this_lifelog['location'], small_location = this_lifelog['small_location'],
