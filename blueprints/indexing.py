...
is_moving=BOOLEAN(stored=True),small_location=KEYWORD(commas=True,stored=True,analyzer=my_ana),
...
shot_ix = index.create_in("index_dir", shot_schema)
...
shot_file.close()
shot_file = open('../../LSC2020_dataset/process_exblur/Shotmrg_newkey.txt','r')
Outfile = open('../../LSC2020_dataset/process_exblur/Shotinfo_search_smalllocation_feb22.txt','w')
cnt = 0
for line in shot_file:
    cnt += 1
    if not line:
    ...
    this_lifelog['small_location'] = ",".join(list(this_lifelog['small_location']))
